...
pub async fn run(params: ClientParams) -> Result<()> {
    ...
    let tap_reader_dev = tap_windows::Device::open(&tap_name)
        .with_context(|| format!("[ERROR] client: failed to open TAP device '{}' (reader)", tap_name))?;
    tap_reader_dev
        .set_status(true)
        .context("[ERROR] client: failed to set TAP status up")?;

    let tap_writer_dev = tap_windows::Device::open(&tap_name)
        .with_context(|| format!("[ERROR] client: failed to open TAP device '{}' (writer)", tap_name))?;
